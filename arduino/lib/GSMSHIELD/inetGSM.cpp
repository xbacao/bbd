...
int InetGSM::http_get(char* url)
{
  ...
  //ir buscar resp code com at+httpaction=
  //gsm.SimpleWriteln("AT+HTTPREAD");

  n = gsm.read_httpaction(method, status_code, data_len);
  if(n != 0){
    return 1;
  }
  if(status_code != 200){
  ...
  //FALTA LER OS DADOS CARALHOOOO PUTA MIERDA! CONO!

}*/

/*
int InetGSM::httpGET(const char* server, int port, const char* path, char rsp[50])
{
  ...
  //as vezes aqui vais bucar logo o resultado (??)
 ...
 delay(50);
  ...
  int http_code = gsm.httpRspCode();

  delay(50);
  	#ifdef DEBUG_ON
		Serial.println("DB:RSP CODE RECV");
	#endif

  if(http_code != 200){
		Serial.print("DB:HTTPCODE");
		Serial.println(http_code);
  	return -1;
  }

  if(gsm.getRestContent(rsp)!= 1){
  	return -2;
  }
...
int InetGSM::httpGET(const char* server, int port, const char* path, char rsp[50])
{
  ...
  gsm.SimpleWriteln("AT+HTTPINIT");
  if(gsm.WaitResp(5000, 50, "OK")!=RX_FINISHED_STR_RECV){
    return 1;
  }

  gsm.SimpleWrite("GET ");
  gsm.SimpleWrite("AT+HTTPPARA=\"URL\",\"http://");
  gsm.SimpleWrite(server);
  gsm.SimpleWrite(":");
  char s_port[5];
  itoa(port, s_port, 10);
  gsm.SimpleWrite(s_port);
  gsm.SimpleWrite(path);
  gsm.SimpleWrite("\"");
  gsm.SimpleWrite(end_c);

  if(gsm.WaitResp(20000, 50, "OK")!=RX_FINISHED_STR_RECV){
    return 2;
  }
...
int InetGSM::configureGPRS(char* domain, char* dom1, char* dom2)
{
   int i=0;
   delay(5000);
...
{
  if (gsm.getStatus()==gsm.IDLE) return -1;
